...
#Computing kernel matrix using given polynomial function n = 3
w, h = len(X), len(X)
...
for i in range(len(X)):
    for j in range(len(X)):
        K[i][j] = (1+(x[i][0]*x[j][0] + y[0][i]*y[0][j]))**3
...
plt.xlabel('x-axis')
plt.ylabel('y-axis')
plt.title("Polynomial Kernel PCA for Centered Data for degree = 3",color='black')
plt.grid()
plt.show()