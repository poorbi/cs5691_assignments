...
#Computing kernel matrix using given function for degree = 2
K1 = [[0 for s in range(w)] for t in range(h)] 

for i in range(len(X)):
    for j in range(len(X)):
        K1[i][j] = (1+(x[i][0]*x[j][0] + y[0][i]*y[0][j]))**2
...
for j in range(4):
        for i in range(len(X)):
            if Z[i][0]==j :
                ...
                plt.title('Polynomial Kernel Cluster k = 4 with degree = 2 (argmax)', color = 'black')
